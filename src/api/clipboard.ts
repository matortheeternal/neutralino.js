...
export async function getFormat(): Promise<ClipboardFormat> {
    return await sendMessage('clipboard.getFormat');
};

export async function readText(): Promise<string> {
    return await sendMessage('clipboard.readText');
};

export async function readImage(format: string = ''): Promise < ClipboardImage | null > {
    const image = await sendMessage('clipboard.readImage');
    if (!image) return;
    ...
    bytes = new Uint8Array(len);
    const isLittleEndian: boolean = new Uint8Array(
...
export async function writeText(data: string): Promise<void> {
    return await sendMessage('clipboard.writeText', { data });
}

export async function writeImage(image: ClipboardImage): Promise<void> {
    const props: any = {...image};
    if(image?.data) {
        props.data = arrayBufferToBase64(image.data);
    }
    return await sendMessage('clipboard.writeImage', props);
}

export async function readHTML(): Promise<string> {
	return await sendMessage('clipboard.readHTML');
}

export async function writeHTML(data: string): Promise<void> {
	return await sendMessage('clipboard.writeHTML', { data });
}

export async function clear(): Promise<void> {
    return await sendMessage('clipboard.clear');
}
